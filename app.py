...
# Afaan Oromo & Amharic Channels (Culture, History, Language, Folklore & News)
DEFAULT_CHANNELS = [
    # Afaan Oromo Channels
    "bbcafaanoromoo",
    "fana_afaan_oromoo",
    "obn_afaan_oromoo",
    "voaafaanoromoo",
    ...
    # Amharic Channels (Culture, History & News)
    "tikvahethiopia",
    "fana_brodcasting",
    "ethiopianhistory1",
    "ethio_culture_arts"
...
BATCH_SIZE = 20  # Batch 20 posts per 1 Gemini AI request
...
def get_active_channels():
    """Fetches preset channels combined with dynamic channels from Firestore if available."""
    channels = set(DEFAULT_CHANNELS)
    global db
    if db:
        try:
            # Fetch custom channels added via Telegram Bot to Firestore
            ...
            print(f"[!] Custom channels fetch error: {e}")
    return list(channels)

# ============ REAL SCRAPING ============

def scrape_telegram_web(channel):
    """Scrapes raw text from Telegram public web view."""
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[!] HTTP Error {response.status_code} for channel: {channel}")
            return []
        ...
        for match in matches:
            ...
            # Filter out noise, link-only or extremely short texts
...
def deduplicate_posts(posts_list):
    """Removes duplicate posts based on unique hash."""
    seen_hashes = set()
    unique_posts = []
...
# ============ ADVANCED GEMINI AI FILTER & HUMANIZER ============

def process_batch_with_gemini(text_batch):
    """Processes 20 texts in 1 single Gemini request with strict security & quality filters."""
...
1. PRIVACY & SECURITY: Completely DISCARD any text containing personal sensitive information (phone numbers, private IDs, private secrets) or national security secrets.
2. QUALITY & DUPLICATES: Filter out spam, promotional ads, repeated headlines, or non-informative text.
3. HUMANIZATION: Clean, humanize, and format the Afaan Oromo text ("text_oromo").
...
6. QUALITY RATING: Assign a score from 0.0 to 10.0 based on linguistic and informative value.

OUTPUT FORMAT:
Return ONLY a strictly valid JSON Array of objects. No markdown formatting outside JSON, no introductory prose.
...
def save_to_firestore(item):
    """Saves high-quality cleaned JSON item to Firestore."""
    global db
    if not db:
...
def send_telegram_report(run_scanned, run_unique, run_saved, duration, total_channels):
    """Sends clean dataset execution report directly to Telegram Bot."""
...
def run_pipeline():
    ...
    print(f"\n[+] Starting Pipeline Run at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    
    # Fetch active channels (50% preset + custom)
    ...
    # 3. Batched Gemini Processing (20 posts per request)
    if unique_texts:
        ...
        for batch in chunks:
            ...
            for item in processed_data:
                try:
                    ...
                    print(f"[!] Failed to save individual item: {e}")
            time.sleep(2)
    ...
    print(f"[+] Pipeline Completed. Scanned: {run_scanned}, Saved: {run_saved}, Duration: {duration}s")
